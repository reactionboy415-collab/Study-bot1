...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "snapstudy_ultra_secret")
...
def admin():
    ...
    <div class="p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-slate-800 p-6 rounded-xl">Total: {stats['total_requests']}</div>
            <div class="bg-slate-800 p-6 rounded-xl text-green-500">Success: {stats['success_count']}</div>
            <div class="bg-slate-800 p-6 rounded-xl text-red-500">Failed: {stats['failed_count']}</div>
        </div>
        <table class="w-full bg-slate-800 rounded-xl overflow-hidden">
            <thead class="bg-slate-700"><tr><th class="p-4">Time</th><th class="p-4">IP</th><th class="p-4">Topic</th><th class="p-4">Status</th></tr></thead>
