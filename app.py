...
VIDEO_STATUS_URL = f"{NOTEGPT_BASE}/api/v2/pdf-to-video"  # same endpoint used for polling (query params)
...
def add_anonymous_cookie(scraper):
    # Generate a fresh UUID for every new request
...
def notegpt_init(scraper, topic):
    ...
    data = resp.json()
    # Adjust according to expected schema; assume data has conversation_id
...
def notegpt_fetch_script(scraper, conversation_id):
    ...
    resp.raise_for_status()
    data = resp.json()
    # Assume script data in data["data"]
...
def notegpt_edit_script(scraper, conversation_id, script_data):
    """
    Append ' | By Chirag Rathi' to each scene text and POST to edit endpoint.
    Assumes script_data has something like:
    {
      "script": {
        "scenes": [
          {"text": "...", ...},
...
def notegpt_poll_video(scraper, conversation_id, timeout_sec=300, interval_sec=8):
    ...
    Assume status endpoint is same base with query parameters.
    """
    start = time.time()
    last_data = None
...
<body class="h-full text-slate-100 scanlines">
    ...
    <div class="w-full max-w-4xl cyber-border rounded-xl bg-slate-900/80 backdrop-blur-md glow">
      ...
      <div class="px-6 py-6 grid gap-6 md:grid-cols-2">
        ...
        <div class="space-y-4">
          <form id="topicForm" class="space-y-4" method="POST" action="/">
            ...
            <div class="flex items-center justify-between">
              <button type="submit"
                ...
                <svg class="w-4 h-4 mr-1 animate-pulse" fill="none" stroke="currentColor" stroke-width="1.8"
                  viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M4 4v5h.582m0 0A7 7 0 0112 5h0a7 7 0 016.418 4.027M4.582 9H9m11 11v-5h-.581m0 0A7 7 0 0112 19h0a7 7 0 01-6.418-4.027M19.418 15H15" />
                </svg>
          ...
          <!-- Debug area -->
          <div id="debugBox"
               class="mt-2 text-xs rounded-md border border-red-500/50 bg-red-950/60 px-3 py-2 text-red-100 {% if not error %}hidden{% endif %}">
            <div class="flex items-center justify-between mb-1">
              <span class="font-semibold">Debug Mode</span>
              <button onclick="document.getElementById('debugBox').classList.add('hidden');"
        ...
        <div class="space-y-4">
          <div class="border border-slate-700 rounded-md bg-slate-950/60 p-3">
            <div class="flex items-center justify-between mb-2">
              ...
              {% if status_message %}
              <span class="text-[11px] text-neon-green">
                {{ status_message }}
              </span>
              {% else %}
            ...
            <p class="mt-2 text-[10px] text-slate-400">
              Branded script: <span class="text-neon-yellow">| By Chirag Rathi</span>
              appended to scenes.
            </p>
            <a href="{{ video_url }}" target="_blank"
               class="inline-flex mt-2 items-center px-2.5 py-1.5 rounded-md bg-slate-800 text-[11px] text-neon-blue border border-slate-600 hover:bg-slate-700">
  ...
  <script>
    // If server returned an error, ensure debug is visible.
    {% if error %}
    ...
    <div class="w-full max-w-5xl cyber-border rounded-xl bg-slate-900/85 backdrop-blur-md">
      ...
      <div class="px-6 py-4 space-y-6">
        <div class="border border-slate-700 rounded-md bg-slate-950/70 p-3">
          ...
          <p class="text-xs text-neon-blue">
            {{ server_ip }}
          </p>
          <p class="text-[11px] text-slate-500 mt-1">
                ...
                {% if logs %}
                  ...
                  <tr class="border-b border-slate-800 hover:bg-slate-900/70">
                    ...
                    <td class="px-3 py-1.5 align-top">
                      ...
                      <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-emerald-900/60 text-emerald-300 border border-emerald-500/70">
                        SUCCESS
                      </span>
                      {% else %}
                      <span class="inline-flex items-center px-2 py-0.5 rounded-full bg-red-900/60 text-red-300 border border-red-500/70">
                        FAIL
                      </span>
                      {% endif %}
                    </td>
        ...
        if not topic:
            ...
            if not check_rate_limit(user_ip):
                ...
                try:
                    ...
                    # Wait 10 seconds as required before fetching script
                    time.sleep(10)
                    ...
                    # 3) Edit & lock
                    _ = notegpt_edit_script(scraper, cid, script_data)
...
if __name__ == "__main__":
    # Typical production deployment will use gunicorn/uwsgi, but keep this for local dev.
