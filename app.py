...
def notegpt_init(scraper, topic):
    ...
    if not cid and "data" in data and isinstance(data["data"], dict):
        cid = data["data"].get("conversation_id")
...
def notegpt_fetch_script(scraper, conversation_id):
    ...
    script_data = data.get("data", data)
    if not script_data:
...
def notegpt_edit_script(scraper, conversation_id, script_data):
    if not isinstance(script_data, dict):
        raise RuntimeError(f"script_data is not a dict: {type(script_data)}")
    ...
    if not isinstance(result, dict):
        raise RuntimeError(f"Edit script API returned non-dict response: {result}")
...
def notegpt_poll_video(scraper, conversation_id, timeout_sec=300, interval_sec=8):
    ...
    while time.time() - start < timeout_sec:
        ...
        if not isinstance(data, dict):
            raise RuntimeError(f"Poll API returned non-dict response: {data}")
        ...
        if status == "success":
            ...
            if "video_url" in data:
                ...
                video_url = data["data"].get("video_url") or data["data"].get("result_url")
            ...
            raise RuntimeError(f"Video generation failed with status '{status}': {data}")
        
        time.sleep(interval_sec)

    raise TimeoutError(f"Polling timed out after {timeout_sec}s. Last response: {last_data}")
