...
# ... (Baaki code same rahega upar wala)
    ...
    try:
        ...
        init = scraper.post("https://notegpt.io/api/v2/pdf-to-video", json={
            ...
            "setting": {
                ...
                "gen_flow": "edit_script", "add_watermark": False # Core request
            }
        ...
        # --- NEW WATERMARK BYPASS LOGIC ---
        # NoteGPT metadata mein force-remove watermark
        if 'setting' in script_data:
            script_data['setting']['add_watermark'] = False
        
        # Force the editor to acknowledge the no-watermark rule
        payload = {
            "conversation_id": cid, 
            "script_data": json.dumps(script_data),
            "is_force_save": True # Force the system to override default watermark
        }

        # Step 3: Trigger No-Watermark Rendering
        scraper.post("https://notegpt.io/api/v2/pdf-to-video/script/edit", 
                     json=payload, headers={'User-Agent': ua}, cookies={'anonymous_user_id': aid})
...
# ... (Rest of the status and admin routes)
